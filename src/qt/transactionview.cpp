    ...
    transactionView(0)
{
    ...
    QAction *viewOnJMAGchain = new QAction(tr("Show transaction on JMAGchain"), this);
    ...
    contextMenu->addSeparator();
    contextMenu->addAction(viewOnJMAGchain);
    ...
    connect(viewOnJMAGchain, SIGNAL(triggered()), this, SLOT(viewOnJMAGchain()));
}
...
void TransactionView::viewOnJMAGchain()
{
    QModelIndexList selection = transactionView->selectionModel()->selectedRows();
    if(!selection.isEmpty())
    {
        QString format("http://JMAGchain.info/tx/");
